...
import safeBase64 from './safe-base64.mjs';
import { SupportedContentEncodings } from './web-push-constants.mjs';
import urlBase64Helper from './urlsafe-base64-helper.mjs';
...
function getVapidHeaders(audience, subject, publicKey, privateKey, contentEncoding, expiration) {
	...
	const jwt = (() => {
		const encodedHeader = safeBase64.from_string(JSON.stringify(header), 'binary');
		const encodedPayload = safeBase64.from_string(JSON.stringify(jwtPayload), 'utf8');
		const securedInput = util.format('%s.%s', encodedHeader, encodedPayload);

		// 暗号化方法はES256固定
		const signer = crypto.createSign('RSA-SHA256');
		signer.update(securedInput);
		const sig = safeBase64.from_base64(signer.sign(toPEM(privateKey), 'base64'));
